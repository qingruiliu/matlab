...
% 2. time length for single trial is saved in the data matrix    check
...
h.sampleF = 48000;
h.audioHandle = PsychPortAudio('Open', 0, 1, 1, h.sampleF, 2);  %use PsychPortAudio('GetDevices') to find UR12, and change the first number with UR12 number
PsychPortAudio('Volume', h.audioHandle, 0.05);      %auditory cue volume
...
f = figure('Name','trial monitor','Position',screenSize);                                  %open the lick monitor at left side of the main monitor
...
uicontrol(f,'Style','text','Units','normalized',...
 ...
 h.hitTrialNumUI= uicontrol(f,'Style','edit','Units','normalized',...
    'Position',[0.3 0.91 0.1 0.03],'FontSize',16); 
...
h.lickInRWtotal = uicontrol(f,'Style','edit','Units','normalized',...
    'Position',[0.5 0.9 0.15 0.04],'FontSize',16); %replace the 0 with callback function
...
uicontrol(f,'Style','text','Units','normalized',...
 ...
 h.lickOutRWtotal = uicontrol(f,'Style','edit','Units','normalized',...
    'Position',[0.7 0.9 0.15 0.04],'FontSize',16); %replace the 0 with callback function
...
h.lickInRWsingle = uicontrol(f,'Style','edit','Units','normalized',...
    'Position',[0.5 0.78 0.15 0.04],'FontSize',16); %replace the 0 with callback function
...
h.lickOutRWSingle = uicontrol(f,'Style','edit','Units','normalized',...
    'Position',[0.7 0.78 0.15 0.04],'FontSize',16); %replace the 0 with callback function
...
% time length monitor
h.trialTime = axes(f,'Position',[0.55 0.44 0.3 0.3]);
...
PsychPortAudio('Close',h.audioHandle); %where to close the 
...
global h
    h.inOrOutRW = 2;
    h.ITITime = tic;
    while toc(h.ITITime) <= 4 %time length of ITI is 4 seconds
    end
