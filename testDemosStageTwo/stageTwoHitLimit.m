...
%% start tic 
totalTic = tic;
h.counterTic = tic;  % tic of the lick counter 
h.inRWCounter = 0;
h.outRWCounter = 0;
h.inOrOutRW = 0; 
...
set(h.lickOutRWSingle,'String',num2str(h.outRWCounterSingle));
    if trialNum <= 300  % within 300 trials (about 50 mins)
        %h.licktrial <= lickTrialLimit  %50min * 50 seconds / 10.1s (one trial) is approximately 297 trials
    disp('------------------new trial-----------------------')
    fprintf('trialNum = %s\n',num2str(trialNum))
...
%         h.lickdata{h.licktrial}(lickTimesReporter,1) = lickTimesReporter;
%         h.lickdata{h.licktrial}(lickTimesReporter,2) = tocReporter;
%      end        
  end
